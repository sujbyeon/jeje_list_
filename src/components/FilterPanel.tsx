...
function TierConnector({ active }: { active: boolean }) {
  return (
    <div className="flex items-stretch">
      <div className={`w-14 md:w-20 shrink-0 flex items-center justify-center border-r border-border bg-secondary/30 relative`}>
        <div className={`absolute top-0 bottom-0 left-1/2 w-0.5 -translate-x-1/2 ${active ? 'bg-gradient-to-b from-primary to-gold' : 'bg-border'}`} />
        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] relative z-10 border-2 transition-all ${active ? 'border-gold text-gold bg-accent/10' : 'border-border text-muted-foreground bg-card'}`}>
          ▼
        </div>
      </div>
